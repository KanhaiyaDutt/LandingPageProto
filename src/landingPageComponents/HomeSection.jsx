...
// Combined hero section with more visible blobs
export default function CombinedHeroSection() {
  
  // VISIBILITY & SIZE TWEAKS: Opacity increased and sizes reduced for all blobs.
  const blobConfigurations = [
    // Larger blobs (now smaller)
...
          <div
            key={index}
            // REDUCED BLUR: Changed from blur-2xl to blur-xl for sharper edges
            className={`absolute rounded-full filter blur-xl ${blob.classes}`}
            style={{
              // EXPANDED GRADIENT: Changed from 80% to 90% for a larger color core
...
          cursor={true}
          repeat={Infinity}
          className="text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold leading-tight mb-4 tracking-wide 
          bg-gradient-to-r from-slate-800 to-indigo-600 text-transparent bg-clip-text"
        />
        <motion.p 
          variants={itemVariants}
          className="text-lg md:text-xl lg:text-2xl text-slate-700 mb-8 max-w-3xl mx-auto"
        >
